...
#img=ImageTk.PhotoImage(file="asset\\backimg.png")

main.iconbitmap('asset\\YoutubeL.ico')
...
back=PhotoImage(file="asset\\ytlogo.png")

btn_Frame = Frame(main)
btn_Frame.place(relheight=1, relwidth=1)

back_ground=Label(btn_Frame,image=back,bg="#5ed94e")
back_ground.place(x=705, y=40)#relheight=1, relwidth=1


lbl_url = tk.Label(btn_Frame, text="YouTube Link", width=5, height=1, fg="red", bg="#ccf2ff",
                   font=('League Spartan', 10, ' bold '))
lbl_url.grid(row=1, column=0, padx=30, pady=20,ipadx=33,ipady=15)

# Enter path label
#file_img=PhotoImage(file="asset\\path1.png")image=file_img,
lbl_path = tk.Label(btn_Frame, text="Select Path", width=5, height=1, fg="red", bg="#ccf2ff",
                    font=('League Spartan', 10, ' bold '))
lbl_path.grid(row=2, column=0, padx=30,ipadx=36,ipady=15)

# Url Entry
url_str = StringVar()
path = ttk.Entry(btn_Frame, width=60, textvariable=url_str, font=('DM Sans', 10))
path.grid(row=1, column=1, ipady=2, padx=0, pady=5, sticky="W", columnspan=2)
...
file.grid(row=2, column=1, ipady=2, padx=0, pady=5,columnspan=2)
# Context Menu(i.e, cut,copy,paste on right click)
file.bind('<Button-3>', rClicker, add='')

# video checkbox
video_icon=PhotoImage(file="asset\\video3.png")
video_check = tk.Checkbutton(btn_Frame,image=video_icon, text="video", onvalue=1, variable=Audio_Video,bg="#f5f7f6",relief="solid",cursor="hand2")
video_check.grid(row=3, column=1, padx=0)

# audio checkbox
audio_icon=PhotoImage(file="asset\\audio2.png")
audio_check = tk.Checkbutton(btn_Frame,image=audio_icon, text="Audio", onvalue=2, variable=Audio_Video,bg="#f5f7f6", relief="solid",cursor="hand2")
audio_check.grid(row=3, column=2, padx=0)


# confirm Button

#confirm_img=PhotoImage(file="asset\\checkvideo1.png")image=confirm_img,
confirm = tk.Button(btn_Frame, text="Search Quality",width=11,height=1,bd=0,background="black",fg="white",cursor="hand2", command=lambda: check_video())
#confirm = tk.Button(btn_Frame, text="Check video",width=85,height=29,bd=0, command=lambda: check_video())
confirm.grid(row=1, column=3, padx=10, pady=5)

# browse Button
#imf=PhotoImage(file="asset\\fileM.png")image=imf,
browse = Button(btn_Frame, text="Browse", width=10,height=1,background="black",fg="white",cursor="hand2", command=lambda: browse_button(),bd=0)
browse.grid(row=2, column=3, padx=6, pady=5)
...
# Download Progress
#download_prog=PhotoImage(file="asset\\downloading.png")image=download_prog,
download_Frame = tk.Label(btn_Frame, text="Downloading Progress:",fg="white",height=1, width=20,bg="black",font=('arial',15)).place(x=700, y=255)
...
box=tk.Label(btn_Frame,text="Select Quality below here:",fg="white",width=20,bg="black",font=("arial",10)).place(x=53,y=217)

devloper_frame=tk.Label(btn_Frame,text="Developed By:",fg="white",height=1,width=20,bg="black",font=("arial",15)).place(x=700, y=555)
devloper_name=tk.Label(btn_Frame,text="Sandeep Kumar",fg="white",height=1,width=20,bg="black",font=("arial",15)).place(x=700, y=585)

# Download Button
#s = ttk.Style()
#s.configure('my.TButton', font=('verdana', 20))
ty=PhotoImage(file="asset\\download1.png")

download_button = tk.Button(main,text="Download",image=ty,bg="#f85646",height=73, width=207,cursor="hand2", command=lambda: th.Thread(target=Download).start(),bd=0)
download_button.place(x=700, y=380)
